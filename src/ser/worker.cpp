...
void* ServerWorker(void* arg)
{
	...
	while(true)
	{
		...
		switch(packet.flag)
		{
			...
			{
				std::string data = DecryptSSL(packet.data, packet.size - 8, secret_key);
				...
				else
				{
					...
					currDirXML = userFiles.FirstChildElement("files");
					assert(currDirXML != nullptr);
					auth = true;
				}
			}
			break;
			...
			case Flags::AUTH_REQUEST:
			{
				...
				if(found)
				{
					...
					if(memcmp(hash, oldhash, 64) == 0)
					{
						...
						auth = true;
						std::cout << userFiles.ChildElementCount() << '\n';
						currDirXML = userFiles.RootElement();
						assert(currDirXML != nullptr);
						break;
					}
				}
			...
			case Flags::SEND_FILE_END:
			{
				...
				XMLElement* file = currDirXML->InsertNewChildElement("file");
			...
			case Flags::DIR_LIST_REQUEST:
			{
				...
				{
					assert(currDirXML != nullptr);
					XMLElement* file = currDirXML->FirstChildElement("file");
					...
					XMLElement* dir = currDirXML->FirstChildElement("dir");
			...
			case Flags::CREATE_DIR:
			{
				...
				XMLElement* newdir = currDirXML->InsertNewChildElement("dir");
			...
			case Flags::CHANGE_DIR:
			{
				...
				if(dirname == "../")
				{
					if(currDirXML->Parent()->ToElement() != nullptr)
					{
						currDirXML = currDirXML->Parent()->ToElement();
						Packet response(Flags::ACCEPT, NULL, 0);
						response.Send(info.clientSocket);
						break;
					}
